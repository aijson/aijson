...
@pytest.mark.parametrize(
    ...
    [
        (
            None,
            "Retrying aijson.repos.blob_repo.S3BlobRepo._wrap_tenacity.<locals>._timeout in 0.0 seconds as it raised InvalidObjectState: An error occurred (000) when calling the mock operation: Unknown.",
        ),
        (
            EndpointConnectionError(endpoint_url="mock"),  # botocore exc
            'Retrying aijson.repos.blob_repo.S3BlobRepo._wrap_tenacity.<locals>._timeout in 0.0 seconds as it raised EndpointConnectionError: Could not connect to the endpoint URL: "mock".',
        ),
        (
            ResourceLoadException(),  # boto3 exc
            "Retrying aijson.repos.blob_repo.S3BlobRepo._wrap_tenacity.<locals>._timeout in 0.0 seconds as it raised ResourceLoadException: .",
        ),
    ],
)
...
async def test_s3_blocking(
    ...
    for log_entry in log_history:
        assert log_entry == {
            "event": "Retrying aijson.repos.blob_repo.S3BlobRepo._wrap_tenacity.<locals>._timeout in 0.0 seconds as it raised TimeoutError: .",
            "exc_info": ANY,
            "log_level": "warning",
